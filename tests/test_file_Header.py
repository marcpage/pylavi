...
def test_Header():
    for header_index, header_bytes in enumerate(TEST_SETS):
        ...
        assert header.file_type == TEST_SETS[header_bytes]['file_type'], f"{header_index}: {[header.file_type, TEST_SETS[header_bytes]['file_type']]}"
        assert header.file_creator == TEST_SETS[header_bytes]['file_creator'], [header.file_creator, TEST_SETS[header_bytes]['file_creator']]
        ...
        assert header == Header().from_bytes(header_bytes), f"header_index = {header_index} {header_bytes}"
        assert header != 5
